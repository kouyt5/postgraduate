...
void process_cli(int connectfd, struct sockaddr_in client) {
   ...
   char  recvbuf[MAXDATASIZE],sendbuf[MAXDATASIZE],client_name[MAXDATASIZE],savebuf[MAXDATASIZE];
	int j;
   for(j=0;j<100;j++)
      savebuf[j]=NULL;
   int recvlen,i;
   ...
   printf("获得一个连接 %s\n",  inet_ntoa(client.sin_addr));                  
    send(connectfd, "欢迎来到chencong的服务器.\n", 22, 0);
   ...
   while(recvlen=recv(connectfd, recvbuf, MAXDATASIZE,0))                 
    {                         
      recvbuf[recvlen]='\0';
      strcat(savebuf,recvbuf);
      printf("从客户端受到信息  '%s' : %s.",client_name,recvbuf);         
      printf("\n");
   for(i=0;i<recvlen;i++)      
